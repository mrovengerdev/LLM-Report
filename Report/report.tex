...
\begin{aligned}
&0 + n = n \\
&\text{induction } n \text{ with } d, \text{ hd} \\
&0 + 0 = 0 \\
&\text{rw}[\text{add\_zero}] \\
&0 = 0 \\
&\text{rfl} \\
&0 + \text{succ } d = \text{succ } d \\
&\text{rw}[\text{add\_succ}] \\
...
&\text{rfl} \\
&\text{succ } a + \text{succ } b = \text{succ } (a + \text{succ } b) \\
&\text{rw}[\text{add\_succ}] \\
&\text{succ } (\text{succ } a + b) = \text{succ } (a + \text{succ } b) \\
&\text{rw}[\text{n\_ih}] \\
&\text{succ } (\text{succ } (a + b)) = \text{succ } (a + \text{succ } b) \\
&\text{rw}[\text{add\_succ}] \\
&\text{succ } (\text{succ } (a + b)) = \text{succ } (\text{succ } (a + b)) \\
&\text{rfl} \\
\end{aligned}
\]
...
\begin{aligned}
&a + b = b + a \\
&\text{induction } b \text{ with } b, \text{ hb} \\
&a + 0 = 0 + a \\
&\text{rw}[\text{add\_zero}] \\
&a = 0 + a \\
&\text{rw}[\text{zero\_add}] \\
&a = a \\
&\text{rfl} \\
&a + \text{succ } b = \text{succ } b + a \\
&\text{rw}[\text{add\_succ}] \\
&\text{succ } (a + b) = \text{succ } b + a \\
&\text{rw}[\text{hb}] \\
&\text{succ } (b + a) = \text{succ } b + a \\
&\text{rw}[\text{succ\_add}] \\
&\text{succ } (b + a) = \text{succ } (b + a) \\
&\text{rfl} \\
\end{aligned}
\]
...
\begin{aligned}
&a + b + c = a + (b + c) \\
&\text{induction } c \text{ with } c, \text{ hc} \\
&a + b + 0 = a + (b + 0) \\
&\text{rw}[\text{add\_zero}] \\
&a + b = a + (b + 0) \\
&\text{rw}[\text{add\_zero}] \\
&a + b = a + b \\
&\text{rfl} \\
&a + b + \text{succ } c = a + (b + \text{succ } c) \\
&\text{rw}[\text{add\_succ}] \\
&\text{succ } (a + b + c) = a + (b + \text{succ } c) \\
&\text{rw}[\text{add\_succ}] \\
&\text{succ } (a + b + c) = a + \text{succ } (b + c) \\
&\text{rw}[\text{add\_succ}] \\
&\text{succ } (a + b + c) = \text{succ } (a + (b + c)) \\
&\text{rw}[\text{hc}] \\
&\text{succ } (a + (b + c)) = \text{succ } (a + (b + c)) \\
&\text{rfl} \\
\end{aligned}
...
In Level 4, the Lean proof establishes the \textit{associativity of addition} through induction. The objective is to demonstrate that  a + (b + c) = (a + b) + c . The proof starts with the \textit{base case}  c = 0 , where the Lean rule  \texttt{add\_zero}  simplifies both sides to show equality. For the \textit{inductive step}, it is assumed that the property holds for a specific  c , and the goal is to prove it for  c + 1 , the \textit{successor of }  c . Lean uses  \texttt{add\_succ}  to rewrite the expression involving the successor function, then applies the inductive hypothesis. The proof concludes by demonstrating that both sides are equal, completing the argument. This approach mirrors the structure of discrete mathematics, where the base case at 0 is proven, and the property is extended to the \textit{successor of}  c.
...
&\text{induction } c \text{ with } c, \text{ hc} \\
&a + b + 0 = a + 0 + b \\
&\text{rw}[\text{add\_zero}, \text{add\_zero}] \\
&a + b = a + b \\
&\text{rfl} \\
&a + b + \text{succ } c = a + \text{succ } c + b \\
&\text{rw}[\text{add\_succ}] \\
&\text{succ } (a + b + c) = a + \text{succ } c + b \\
&\text{rw}[\text{add\_succ}] \\
&\text{succ } (a + b + c) = \text{succ } (a + c) + b \\
&\text{rw}[\text{succ\_add}] \\
&\text{succ } (a + b + c) = \text{succ } (a + c + b) \\
&\text{rw}[\text{hc}] \\
&\text{succ } (a + c + b) = \text{succ } (a + c + b) \\
&\text{rfl} \\
\end{aligned}
\]
